...
import { useLpjBendaharaDocuments, useDeleteLpjBendahara } from "./use-lpj-bendahara-documents";
import type { Document } from "@/components/documents/DocumentsSection";

export function useLpjBendaharaTable(year: string, itemsPerPage = 10) {
  ...
  const { data: documents = [], isLoading, isError } = useLpjBendaharaDocuments(year);
  const deleteMutation = useDeleteLpjBendahara();
