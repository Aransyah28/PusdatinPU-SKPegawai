...
import type { LakipDocumentRow } from "@/lib/lakip/lakip-queries";
import type { Document } from "@/components/documents/DocumentsSection";

export function useLakipTable(folderSlug: string, itemsPerPage = 10) {
  ...
  const { data: documents = [], isLoading, isError } = useQuery({
    queryKey: ["lakip-documents", folderSlug],
    queryFn: async () => {
      const res = await fetch(`/api/lakip/folders/${encodeURIComponent(folderSlug)}/documents`);
      if (!res.ok) throw new Error("Gagal mengambil data dokumen");
      return res.json() as Promise<LakipDocumentRow[]>;
    },
  });

  const deleteMutation = useMutation({
    mutationFn: async (documentId: string) => {
      const res = await fetch(`/api/lakip/documents/${documentId}`, {
        method: "DELETE",
      });
      if (!res.ok) {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["lakip-documents", folderSlug] });
    },
  });
