...
import { useRkaklDocuments, useDeleteRkakl } from "./use-rkakl-documents";
import type { Document } from "@/components/documents/DocumentsSection";

export function useRkaklTable(year: string, itemsPerPage = 10) {
  ...
  const { data: documents = [], isLoading, isError } = useRkaklDocuments(year);
  const deleteMutation = useDeleteRkakl();
